...
const Leikkaus = () => {
            ...
            .then(response => {
                ...
                if (Array.isArray(responseData)) {
                    const cleanedData = responseData.map(row => ({
                        ...
                        "StatusLeikkaus": row["StatusLeikkaus"] ?? 'Ei aloitettu',
                        "total_madeLeikkaus": row["total_madeLeikkaus"] ?? '-',
                        "section": osasto,
                        "Sonumero": row["Sales order"]?? "Ei ole",
    ...
    const handleRowClick = (row) => { 
        navigate('/StartWork', { state: {...row, section: "Leikkaus" }});
    };
    ...
    const handleHistory = (row) => {
        ...
        axios.post(`${API_URL}/api/fetch_history`, { id: selectedKey, section: "Leikkaus" }, {
            headers: {
    ...
    const getStatusColor = (StatusLeikkaus) => {
        switch (StatusLeikkaus) {
            case 'Aloitettu':
                            ...
                            {filteredData.length > 0 ? (
                                filteredData.map((row, index) => (
                                    <TableRow
                                        ...
                                        sx={{
                                            cursor: 'pointer',
                                            bgcolor: getStatusColor(row["StatusLeikkaus"]),
                                            '&:hover': {
                                    ...
                                    >
                                        ...
                                        <TableCell sx={{ padding: '6px 10px', fontSize: '0.875rem' }} >{row["Sonumero"]}</TableCell>
                                        ...
                                        <TableCell sx={{ padding: '6px 10px', fontSize: '0.875rem' }} >{row["total_madeLeikkaus"]}</TableCell>
                                        <TableCell sx={{ padding: '6px 10px', fontSize: '0.875rem' }} >{row["StatusLeikkaus"]}</TableCell>
                                        <TableCell>
                    ...
                    {isEditing && selectedTask && (
                        <Box sx={{ mt: 3 }}>
                            ...
                            <FormControl fullWidth sx={{ mt: 2 }}>
                                ...
                                >
                                    {phases
                                        .filter(phase => phase.id === "Leikkaus") // ✅ Filter for "Leikkaus" phases
                                        .map((phase, index) => (
