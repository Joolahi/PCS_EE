...
const Remmit = () => {
    ...
    const [selectedField, setSelectedField] = useState (100);
    ...
    const fetchData = (osasto) => {
        setLoading(true);
        axios.get(`${API_URL}/api/getOsasto${osasto}`,{wirthCredentials: true})    
            .then((response) => {
                setLoading(false);
                ...
                if (Array.isArray(responseData)) {
                    const cleanedData = responseData.map(row => ({
                        ...
                        "StatusRemmit": row["StatusRemmit"] ?? 'Ei aloitettu',
                        "total_madeRemmit": row["total_madeRemmit"] ?? '-',
                        "section": osasto,
    ...
    const handleRowClick = (row) => {
        navigate('/StartWork', {state: {...row, section: "Remmit"}})
    };
    ...
    const handleHistory = (row) => {
        ...
        axios.post(`${API_URL}/api/fetch_history`, { id: selectedKey, section: "Remmit" }, {
            headers: {
    ...
    const handleModifyTask = () => {
        if (!selectedTask || newKplDone === "" || newPhase === "") {
            setSnackbarMessage("Fill in all fields before modifying the task.");
            setSnackbarSeverity("error");
            return;
        }
        setUpdating(true);
    ...
    const getStatusColor = (StatusRemmit) => {
        switch (StatusRemmit) {
            case 'Aloitettu':
                    ...
                    <Table stickyHeader size='small'>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell sx={{fontWeight: 'bolder', padding: '6px 10px', fontSize: '0.875rem' }} >{t('item number')}</TableCell>
                            ...
                            {filteredData.length > 0 ? (
                                filteredData.map((row, index) => (
                                    <TableRow
                                        ...
                                        sx={{
                                            cursor: 'pointer',
                                            bgcolor: getStatusColor(row['StatusRemmit']),
                                            '&:hover': {
                                    ...
                                    >
                                        ...
                                        <TableCell sx={{ padding: '6px 10px', fontSize: '0.875rem' }} >{row["total_madeRemmit"]}</TableCell>
                                        <TableCell sx={{ padding: '6px 10px', fontSize: '0.875rem' }} >{row["StatusRemmit"]}</TableCell>
                                        <TableCell>
                                            <Button
                                                onClick={(event) => {
                    ...
                    {isEditing && selectedTask && (
                        <Box sx={{ mt: 3 }}>
                            ...
                            <FormControl fullWidth sx={{ mt: 2 }}>
                                ...
                                >
                                    {phases
                                        .filter(phase => phase.id === "Remmit") // ✅ Filter for "Leikkaus" phases
                                        .map((phase, index) => (
...
export default Remmit;


// Raja arvot +10 => punainen 
