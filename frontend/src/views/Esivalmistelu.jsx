...
const Esivalmistelu = () => {
            ...
            .then(response => {
                ...
                if (Array.isArray(responseData)) {
                    const cleanedData = responseData.map(row => ({
                        ...
                        "StatusEsivalmistelu": row["StatusEsivalmistelu"] ?? 'Ei aloitettu',
                        "total_madeEsivalmistelu": row["total_madeEsivalmistelu"] ?? '-',
                        "section": osasto,
    ...
    const handleRowClick = (row) => { 
        navigate('/StartWork', { state: {...row, section: "Esivalmistelu" }});
    };
    ...
    const handleHistory = (row) => {
        ...
        axios.post(`${API_URL}/api/fetch_history`, { id: selectedKey, section: "Esivalmistelu" }, {
            headers: {
    ...
    const getStatusColor = (StatusEsivalmistelu) => {
        switch (StatusEsivalmistelu) {
            case 'Aloitettu':
                            ...
                            {filteredData.length > 0 ? (
                                filteredData.map((row, index) => (
                                    <TableRow
                                        ...
                                        sx={{
                                            cursor: 'pointer',
                                            bgcolor: getStatusColor(row["StatusEsivalmistelu"]),
                                            '&:hover': {
                                    ...
                                    >
                                        ...
                                        <TableCell sx={{ padding: '6px 10px', fontSize: '0.875rem' }} >{row["total_madeEsivalmistelu"]}</TableCell>
                                        <TableCell sx={{ padding: '6px 10px', fontSize: '0.875rem' }} >{row["StatusEsivalmistelu"]}</TableCell>
                                        <TableCell>
                    ...
                    {isProductReady && (
                        ...
                        <Box sx={{ mt: 3 }}>
                            ...
                            <FormControl fullWidth sx={{ mt: 2 }}>
                                ...
                                >
                                    {phases
                                        .filter(phase => phase.id === "Esivalmistelu") // ✅ Filter for "Leikkaus" phases
                                        .map((phase, index) => (
